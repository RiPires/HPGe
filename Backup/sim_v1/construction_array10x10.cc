...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    G4double xDet = 0.5*xWorld/nCols;//minor detector lenght
    G4double yDet = 0.5*yWorld/nRows;//minor detector height
    ...
    //   Defines a physical detector adding coordinates to the logical detector  //
        ...
        /*{
            ...
            {
            physDetector = new G4PVPlacement(0, G4ThreeVector(-0.25*m+(i+0.5)*m/(2*nRows), -0.25*m+(j+0.5)*m/(2*nCols), 0.45*m), logicDetector, "physDetector", logicWorld, false, j+i*nCols, true);
            }
        }*/
